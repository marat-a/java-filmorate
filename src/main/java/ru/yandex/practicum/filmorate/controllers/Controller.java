...
abstract public class Controller<T extends Entity> {
    ...
    @PostMapping
    public ResponseEntity create(@Valid @RequestBody T object, HttpServletRequest request) {
    ...
    @PutMapping
    public ResponseEntity update(@Valid @RequestBody T object, HttpServletRequest request)  {
        logRequest(object, request);
        try {
            if (validate(object)&&map.containsKey(object.getId())){
                map.put(object.getId(), object);
